...
% fea_dir = '../features/';
% fea_ext = '.htk';
fid = fopen('../../TIMIT_corpus\timit_feaExtraCode/speaker_model_maps.lst', 'rt');
C = textscan(fid, '%s %s');  %分别读取人名、特征名
fclose(fid);
...
% fea_dir = '../features/';
% fea_ext = '.htk';
trial_list = '../../TIMIT_corpus\timit_feaExtraCode/trials.lst';
fid = fopen(trial_list, 'rt');
C = textscan(fid, '%s %s %s');  % 分别读取人名、特征名、标签
fclose(fid);
